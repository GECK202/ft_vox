...
public:
	...
	inline unsigned short get(int x, int y, int z){
		return (map[y*CHUNK_SIZE_D*CHUNK_SIZE_W+z*CHUNK_SIZE_W+x]);
	}

	inline unsigned char get(int x, int y, int z, int channel){
		return (map[y*CHUNK_SIZE_D*CHUNK_SIZE_W+z*CHUNK_SIZE_W+x] >> (channel << 2)) & 0xF;
	}

	inline unsigned char getR(int x, int y, int z){
		return map[y*CHUNK_SIZE_D*CHUNK_SIZE_W+z*CHUNK_SIZE_W+x] & 0xF;
	}

	inline unsigned char getG(int x, int y, int z){
		return (map[y*CHUNK_SIZE_D*CHUNK_SIZE_W+z*CHUNK_SIZE_W+x] >> 4) & 0xF;
	}

	inline unsigned char getB(int x, int y, int z){
		return (map[y*CHUNK_SIZE_D*CHUNK_SIZE_W+z*CHUNK_SIZE_W+x] >> 8) & 0xF;
	}

	inline unsigned char getS(int x, int y, int z){
		return (map[y*CHUNK_SIZE_D*CHUNK_SIZE_W+z*CHUNK_SIZE_W+x] >> 12) & 0xF;
	}

	inline void setR(int x, int y, int z, int value){
		const int index = y*CHUNK_SIZE_D*CHUNK_SIZE_W+z*CHUNK_SIZE_W+x;
