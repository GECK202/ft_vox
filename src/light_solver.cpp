...
void LightSolver::add(int x, int y, int z, int emission) {
	...
	chunk->modified = true;
	chunk->lightmap->set(entry.x-chunk->x*CHUNK_SIZE_W, entry.y-chunk->y*CHUNK_SIZE_H, entry.z-chunk->z*CHUNK_SIZE_D, channel, entry.light);
}
...
void LightSolver::remove(int x, int y, int z) {
	...
	int light = chunk->lightmap->get(x-chunk->x*CHUNK_SIZE_W, y-chunk->y*CHUNK_SIZE_H, z-chunk->z*CHUNK_SIZE_D, channel);
	if (light == 0){
		return;
	}
	...
	chunk->lightmap->set(entry.x-chunk->x*CHUNK_SIZE_W, entry.y-chunk->y*CHUNK_SIZE_H, entry.z-chunk->z*CHUNK_SIZE_D, channel, 0);
}
		...
		for (size_t i = 0; i < 6; i++) {
			...
			if (chunk) {
				...
				if (light != 0 && light == entry.light-1){
					...
					chunk->lightmap->set(x-chunk->x*CHUNK_SIZE_W, y-chunk->y*CHUNK_SIZE_H, z-chunk->z*CHUNK_SIZE_D, channel, 0);
					chunk->modified = true;
				}
		...
		for (size_t i = 0; i < 6; i++) {
			...
			if (chunk) {
				...
				if (block->lightPassing && light+2 <= entry.light){
					chunk->lightmap->set(x-chunk->x*CHUNK_SIZE_W, y-chunk->y*CHUNK_SIZE_H, z-chunk->z*CHUNK_SIZE_D, channel, entry.light-1);
					chunk->modified = true;
					lightentry nentry;
					nentry.x = x;
					nentry.y = y;
