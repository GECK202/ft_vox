...
ChunksController::ChunksController(Chunks* chunks, Lighting* lighting, int seed) : chunks(chunks), lighting(lighting){
	loadersCount = std::thread::hardware_concurrency() - 1;
	if (loadersCount <= 0)
		loadersCount = 1;
	loaders = new ChunksLoader*[loadersCount];
	for (int i = 0; i < loadersCount; i++){
		loaders[i] = new ChunksLoader(seed);
	}
...
bool ChunksController::_buildMeshes(VoxelRenderer* renderer) {
	const int w = chunks->w;
	const int h = chunks->h;
	const int d = chunks->d;
