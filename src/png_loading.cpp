...
int _png_load(const char* file, int* width, int* height, unsigned int target){
    FILE *f;
    ...
    *height = t_height;
    ...
    if (target > 5) {
        ...
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + target, 0, GL_RGBA, t_width, t_height, 0, \
            alpha, GL_UNSIGNED_BYTE, (GLvoid *) image_data);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    }
...
Texture* load_texture(std::string filename){
	int width, height;
	GLuint texture = _png_load(filename.c_str(), &width, &height, 6);
	if (texture == 0){
...
Cubemap* load_cubemap(std::vector<std::string> faces){
    unsigned int textureID;
    glGenTextures(1, &textureID);
    glBindTexture(GL_TEXTURE_CUBE_MAP, textureID);
	int width, height;
    for (unsigned int i = 0; i < faces.size(); i++) {
	    _png_load(faces[i].c_str(), &width, &height, i);
    }
