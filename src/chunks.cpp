...
voxel* Chunks::get(int x, int y, int z){
	...
	y -= oy * CHUNK_SIZE_H;
	z -= oz * CHUNK_SIZE_D;
	int cx = x / CHUNK_SIZE_W;
	int cy = y / CHUNK_SIZE_H;
	int cz = z / CHUNK_SIZE_D;
	if (x < 0) cx--;
	if (y < 0) cy--;
	if (z < 0) cz--;
	...
	if (chunk == nullptr)
		return nullptr;
	int lx = x - cx * CHUNK_SIZE_W;
	int ly = y - cy * CHUNK_SIZE_H;
	int lz = z - cz * CHUNK_SIZE_D;
	return &chunk->voxels[(ly * CHUNK_SIZE_D + lz) * CHUNK_SIZE_W + lx];
}
...
void Chunks::set(int x, int y, int z, int id){
	...
	int ly = y - cy * CHUNK_SIZE_H;
	int lz = z - cz * CHUNK_SIZE_D;
	chunk->voxels[(ly * CHUNK_SIZE_D + lz) * CHUNK_SIZE_W + lx].id = id;
	chunk->modified = true;
	...
	if (lx == CHUNK_SIZE_W-1 && (chunk = getChunk(cx+ox+1, cy+oy, cz+oz))) chunk->modified = true;
	if (ly == CHUNK_SIZE_H-1 && (chunk = getChunk(cx+ox, cy+oy+1, cz+oz))) chunk->modified = true;
	if (lz == CHUNK_SIZE_D-1 && (chunk = getChunk(cx+ox, cy+oy, cz+oz+1))) chunk->modified = true;
}
	...
	for (unsigned int y = 0; y < h; y++){
		for (unsigned int z = 0; z < d; z++){
			for (unsigned int x = 0; x < w; x++){
				...
				if (nx < 0 || ny < 0 || nz < 0 || nx >= (int)w || ny >= (int)h || nz >= (int)d){
