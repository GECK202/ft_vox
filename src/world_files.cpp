...
void WorldFiles::put(const char* chunkData, int x, int y){
	...
	u_tempcoords.coords[0] = regionX;
	u_tempcoords.coords[1] = regionY;
	char** region = regions[u_tempcoords.key];
	if (region == nullptr){
		region = new char*[REGION_VOL];
		for (unsigned int i = 0; i < REGION_VOL; i++)
			region[i] = nullptr;
		regions[u_tempcoords.key] = region;
	}
...
bool WorldFiles::readChunk(int x, int y, char* out){
	...
	decompressRLE(mainBufferIn, compressedSize, out);
...
void longToCoords(int& x, int& y, long key) {
	u_tempcoords.key = key;
	x = u_tempcoords.coords[0];
	y = u_tempcoords.coords[1];
}
