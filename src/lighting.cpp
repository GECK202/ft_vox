...
void Lighting::onChunkLoaded(int cx, int cy, int cz, bool sky){
	...
	if (chunkLower && sky){
		for (int z = 0; z < CHUNK_SIZE_D; z++){
			for (int x = 0; x < CHUNK_SIZE_W; x++){
				int gx = x + cx * CHUNK_SIZE_W;
				int gz = z + cz * CHUNK_SIZE_D;
				...
				if (light < 15){
					...
					for (int y = 15;;y--){
						...
						if (!current)
							break;
						voxel* vox = &(current->voxels[(y * CHUNK_SIZE_D + z) * CHUNK_SIZE_W + x]);//chunks->get(gx,gy+y,gz);
						Block* block = Block::blocks[vox->id];
						if (!block->skyLightPassing)
							break;
						//current->lightmap->setS(x,y,z, 0);
						current->modified = true;
						solverS->remove(gx,y+ncy*CHUNK_SIZE_H,gz);
						current->lightmap->setS(x,y,z, 0);
					}
				}
			}
			...
			for (int x = 0; x < CHUNK_SIZE_W; x++){
				...
				if (light == 15){
					for (int y = CHUNK_SIZE_H-1;;y--){
						...
						current->modified = true;
						solverS->add(gx,y+ncy*CHUNK_SIZE_H,gz);
					}
				} else if (light){
					solverS->add(gx,gy+CHUNK_SIZE_H,gz);
				}
			}
		}
	} else if (sky){
	...
	for (unsigned int y = 0; y < CHUNK_SIZE_H; y++){
		for (unsigned int z = 0; z < CHUNK_SIZE_D; z++){
			for (unsigned int x = 0; x < CHUNK_SIZE_W; x++){
				voxel vox = chunk->voxels[(y * CHUNK_SIZE_D + z) * CHUNK_SIZE_W + x];
				Block* block = Block::blocks[vox.id];
				if (block->emission[0] || block->emission[1] || block->emission[2]){
					int gx = x + cx * CHUNK_SIZE_W;
					int gy = y + cy * CHUNK_SIZE_H;
					int gz = z + cz * CHUNK_SIZE_D;
	...
	for (int y = -1; y <= CHUNK_SIZE_H; y++){
		for (int z = -1; z <= CHUNK_SIZE_D; z++){
			for (int x = -1; x <= CHUNK_SIZE_W; x++){
				if (!(x == -1 || x == CHUNK_SIZE_W || y == -1 || y == CHUNK_SIZE_H || z == -1 || z == CHUNK_SIZE_D))
					continue;
				int gx = x + cx * CHUNK_SIZE_W;
				int gy = y + cy * CHUNK_SIZE_H;
				int gz = z + cz * CHUNK_SIZE_D;
